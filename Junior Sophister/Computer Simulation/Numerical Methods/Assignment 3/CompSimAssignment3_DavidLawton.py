...
# Indexing for the subplots
title_indexing = ['(a)', '(b)', '(c)']
...
for dt in dt_vals:
    ...
    fig.suptitle(f"Euler Method Approximation, $\Delta t = {dt}s$")
    ...
    for i in range(len(t_f_vals)):
        ...
        ax[i].set_title(f"{title_indexing[i]} $t_f = {t_f_vals[i]}s$")
        ax[i].set_xlabel("$t~(s)$")
        ax[i].set_ylabel("$x~(m)$")

    plt.savefig(f"/home/dj-lawton/Documents/Junior Sophister/Computer Simulation/Numerical Methods/Assignment 3/Euler_Method_Approximation_{dt}.pdf")
    plt.show()
    plt.close()
...
for dt in dt_vals:
    fig, ax = plt.subplots(1,2)
    fig.set_size_inches(14, 6)  
    fig.suptitle(f"Higher Order Approximations, $\Delta t = {dt}s$")
    ...
    plt.savefig(f"/home/dj-lawton/Documents/Junior Sophister/Computer Simulation/Numerical Methods/Assignment 3/Higher_Order_Approximations_{dt}.pdf")
    plt.show()
    plt.close()
